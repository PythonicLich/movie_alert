...
def movie_page_scraping(url):
    ...
    directors = [x.text for x in casting_page.html.find(css.values["css_directors"])]
    writers = [x.text for x in casting_page.html.find(css.values["css_writers"])]
    actors = [x.text for x in casting_page.html.find(css.values["css_actors"])]
    session.close()
